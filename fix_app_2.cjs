...
const searchBlock = `              {deliveryType === 'delivery' && (
                <div>
...
const replaceBlock = `              {deliveryType === 'delivery' && (
                <div>
                  <label className="block text-stone-600 font-semibold mb-1">Địa chỉ giao hàng tận tay</label>
                  <input 
                    type="text" 
...
if (code.includes(searchBlock)) {
  ...
  console.log('Search block not found. Trying flexible regex...');
  
  // Create a regex to match from deliveryType to the opening of Modal 2 Sandbox
  const regex = /\{\s*deliveryType === 'delivery' && \(\s*<div>\s*<label[^>]*>Địa chỉ giao hàng tận tay<\/label>\s*<input[^>]*\/>\s*<\/div>\s*<button[\s\S]*?Xác nhận Đặt hàng & Ship ngay\s*<\/button>\s*<\/div>\s*\) : \(\s*\/\/\s*---\s*THE NEW ADVANCED OFFICIAL \/ SIMULATED SANDBOX FLOW\s*---\s*<div className="grid grid-cols-1 md:grid-cols-12 gap-6 text-left">/;
  
  if (regex.test(code)) {
    code = code.replace(regex, replaceBlock);
    fs.writeFileSync('src/App.tsx', code);
    console.log('Successfully repaired App.tsx using regex');
  } else {
    console.error('Failed to find matching block even with regex!');
  }
}
